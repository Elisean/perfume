        ...
        <Breadcrumbs />
          ...
          <div className={openReviews? 'open-review' : 'close-review'}>
           ...
           <Modal isOpen={openModal} onClose={toggleModal} modalTitle={product.title}>
              ...
              <Bonuses className='bonuses-icon'/>
                <p className='bonuses-description'>За текстовый отзыв Вы получите 100 бонусных баллов <br /> 
                За отзыв с фото 150 бонусных баллов <br /> 
                За видео-отзыв с фото 200 бонусных баллов
                </p>
              </div>
            <form ref={formText} action="#" className='modal-form' onSubmit={send}>
              <div className='modal-rating'>
                {/* Переделать */}
                  {
                    [...new Array(5)].map((star, id) => (
                      <div className='star-icon' key={id} > 
                            <Star />
                      </div>
                    ))
                  }

                <p className='rating-title'>Оцените покупку*</p>
                
              </div>
             
                <label htmlFor="modal-review" className='modal-review'>

                 Отзыв*

                  <textarea id="review" value={textValue} onChange={(event) => setTextValue(event.target.value)}/>
                </label>

                <p className='modal-download-title'>Добавьте фотографии (до 3 штук)</p>

               <div className='modal-download-wrapper'>
                  <label htmlFor="modal-photo" className='modal-content-download'>
                      <UserPhoto/>
                      <input id="modal-photo" type='file'/>
                  </label>
                  <div className='modal-user-image-wrapper'>
                  {[...new Array(3)].map(item =>
                    <div className='modal-user-image'></div>
                    )}
                  </div>
                  
               </div> 
               <p className='modal-download-title'>Добавьте видео (до 3 штук)</p>
               ...
               <div className='modal-download-wrapper'>
                ...
                <Button type='submit' padding='12px 20px' >Отправить отзыв</Button>
